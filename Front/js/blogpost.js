import {getDoc,db,doc, collection,where,query,getDocs} from './firebase.js'
...
const getdata1 = async ()=>{
 let currentData = await getDoc(doc(db, "Blogs", id));
        ...
        main.innerHTML =`<div class="post-img">
        <img src="${datavalue.image}" alt="">
    </div>
    <div class="m-auto blog-post-content max-width-2 m-auto my-2">
        <h1 class="font1">${datavalue.BlogName}</h1>
        <div class="blogpost-meta">
            <div class="author-info">
                <div>
                <b>
                  Author - ${datavalue.user}
                </b>
                </div>
...
const peopledata = async() =>{
       let count = 0
        ...
        querySnapshot.forEach((doc) => {
                ...
                if(count < 4){
                        let data = doc.data();
                        row.innerHTML += ` <div class="home-article more-post">
                        <div class="home-article-img">
                            <img src="${data.image}" alt="article">
                        </div>
                        <div class="home-article-content font1 center">
                            <a><h3 onclick="edit('${doc.id}' ,this)">${data.BlogName}</h3></a>
                            
                            <div>Written by ${data.user}</div>
                        </div>
                    </div>`
                }else{
                        return count
